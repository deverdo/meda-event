...
export default class EventScheduleController {
  ...
  async GetEventSchedulesForPreviewBasedOnTag(
  ...
  async GetLatestScheduledEventForPreview(
    req:Request,
    res:Response,
    next:NextFunction
  ){
    try {
      const latestEvents = await prisma.eventSchedule.findMany({
        orderBy:[
      ...
      return apiErrorHandler(error,req,res,"couldnot find event")
    }
  }

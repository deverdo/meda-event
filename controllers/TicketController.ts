...
export default class TicketController {
  ...
  async createTicket2(req:Request, res:Response,next:NextFunction) {
    ...
    try{
      ...
      return apiErrorHandler(err, req, res, "Couldn't create ticket.");
    }
  ...
  async TicketBoughtMedaPayCallback(
    ...
    try {
      ...
      if (status == 'SUCCESS') {
        ...
        // STOP THE SCHEDULE TO REMOVE THE SEAT FROM RESERVATION IF IT IS RUNNING
        //scheduler.removeById(referenceNumber);
        let updatedEvent = await prisma.eventTicket.update({
          where: {
            id: orderId,
          },
          data: {
